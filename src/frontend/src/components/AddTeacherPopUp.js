...
`;
const AddTeacherPopup = ({ onClose, handleUpdateTeachersList }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch('http://localhost:3333/teachers/add', {
                method: 'POST',
                headers: {
            ...
            handleUpdateTeachersList(); // Atualiza a lista de professores no componente pai
    ...
    return (
        <PopupOverlay>
            <PopupContent>
                <TitleClass>Adicionar professor(a)</TitleClass>
